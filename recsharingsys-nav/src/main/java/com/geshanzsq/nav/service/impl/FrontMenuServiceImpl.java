...
import com.geshanzsq.nav.utils.ItemBasedCF;
...
public class FrontMenuServiceImpl implements IFrontMenuService {
     ...
     */
    @Override
    public void updateFrontNavRedis(String userId) {
        // 更新前台导航菜单
...
//        List<FrontSiteVO> siteList = frontMenuMapper.findFrontAllSite(CommonStatus.OK.code);
        List<FrontSiteVO> siteList = recommendSites(userId);
    ...
    @Override
    public List<FrontSiteVO> recommend(){
        List<FrontSiteVO> siteList = frontMenuMapper.findFrontAllSite(CommonStatus.OK.code);

//        int size = siteList.size();
        //从siteList中随机取出8个站点
        List<FrontSiteVO> recommendSiteList = new ArrayList<>();
        Random random = new Random();
        for (int i = 0; i < 8; i++) {
            int index = random.nextInt(siteList.size());
            FrontSiteVO siteVO = siteList.get(index);

            //创建一个FrontSiteVO对象，它的属性值与siteVO一样
            FrontSiteVO site = new FrontSiteVO();
            site.setSiteId(siteVO.getSiteId());
            site.setSiteName(siteVO.getSiteName());
            site.setSiteUrl(siteVO.getSiteUrl());
            site.setSiteDescription(siteVO.getSiteDescription());
            site.setSitePath(siteVO.getSitePath());
            site.setOrderNum(siteVO.getOrderNum());
            site.setMenuId(102L);
    ...
    @Override
    public List<FrontSiteVO> recommendSites(String userId) {

        List<FrontSiteVO> siteList = frontMenuMapper.findFrontAllSite(CommonStatus.OK.code);

        itemBasedCF.getDataset();
        itemBasedCF.calcMovieSimilarity();
        List<Map.Entry<String, Double>> recommend = itemBasedCF.recommend(userId);

        //遍历recommend
        for (Map.Entry<String, Double> entry : recommend) {
     ...
     */
    @Override
    public List<FrontMenuVO> getSystemNavMenu(String userId) {
        //每次只有在修改menu或site的表格时才会触发Redis更新
        //这里使用update是为了手动更新Redis中的数据，
        updateFrontNavRedis(userId);
    ...
    public List<FrontMenuVO> getSystemNavSite(List<FrontMenuVO> menuList) {
        ...
        //获取所有导航站点
        List<FrontSiteVO> siteList = frontMenuMapper.findFrontAllSite(CommonStatus.OK.code);
//        List<FrontSiteVO> siteList = recommend();
